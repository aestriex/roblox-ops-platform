module Components::SearchHelper
  def search_icon(&block)
    content_for :search_icon, capture(&block), flush: true
  end
  ...
  def render_search(items, **options, &block)
    ...
    render "components/ui/search", items: items, options: options, input_class: input_class, content: content
  end

  def search_list_item(value:, name:, selected:, avatar: nil)
    content_tag :div,
